...
node {
  ...
  stage('Build Container') {
    docker.withRegistry('https://docker.fs.usda.gov', 'docker_registry') {
      ...
      def image = docker.build("docker.fs.usda.gov/fia/evalidator_dev:${env.POM_VERSION}", '--no-cache --pull .')
  ...
  stage('Push to cluster') {
     ...
     kubernetesDeploy configs: 'deploy/dev-evalidator.yaml', kubeConfig: [path: ''], kubeconfigId: 'kube_config', secretName: '', ssh: [sshCredentialsId: '*', sshServer: ''], textCredentials: [certificateAuthorityData: '', clientCertificateData: '', clientKeyData: '', serverUrl: 'https://']
  }
